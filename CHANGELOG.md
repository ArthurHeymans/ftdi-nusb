...
- **Cancellation safety.** Dropping the future of a read preserves the
  ...
  input. Dropping a stateful operation (MPSSE transaction, EEPROM
  read/write/erase, mode or baud-rate change, streaming session)
  poisons the device: further I/O returns `Error::RecoveryRequired`
  until `FtdiDevice::recover().await` succeeds
- `FtdiDevice::recover()` drains endpoint transfers, resets the device,
  and re-applies baud rate and bitbang mode (with the recorded direction
  mask). Recovery invalidates existing MPSSE contexts and bus objects,
  which then fail with `Error::InvalidMpsseContext`
